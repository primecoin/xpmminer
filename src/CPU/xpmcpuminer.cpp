...
void sieveL1ExtBenchmark(double difficulty) {
    ...
    for (unsigned i = 1; i <= 8; i++) {
        ...
        // Fast checking CSieveOfEratosthenesL1Ext output
        {
            ...
            l1ext->reset(
                sieveSize, (unsigned)difficulty, gWeaveDepth, fixedMultiplier);
            hp->Reset(
                sieveSize,
                gWeaveDepth,
                HpSieveExtensionsNum,
                HpSieveL1CacheSize,
                header.bits,
                blockHeaderHash,
                primorial,
            ...
            l1ext->Weave();
            hp->Weave();
            if (l1ext->GetCandidateCount() != hp->GetCandidateCount() ||
                !l1ext->fastSelfTest()) {
        ...
        fprintf(
            stderr,
            "(%u, real %u) L1Ext(time %.3lfms speed %.3lf) HP(time %.3lfms speed %.3lf) %.3lf%% faster\n",
            sieveSize,
            realSieveSize,
            tm,
            realSieveSize / tm,
...
void fermatTestBenchmark(double difficulty) {
    ...
    while (updateBlock(&header, blockHeaderHash, primeSource, testParams)) {
        ...
        std::unique_ptr<CSieveOfEratosthenesL1Ext> sieve(
            new CSieveOfEratosthenesL1Ext(primeSource));
        sieve->reset(
            gSieveSize, (unsigned)difficulty, gWeaveDepth, fixedMultiplier);
        sieve->Weave();
        ...
        for (size_t i = 0; i < primesForTest.size(); i++) {
            ...
            FermatProbablePrimalityTestFast(
                primesForTest[i], chainLength, testParams);
        }
...
int main(int argc, char** argv) {
            ...
            case 0:
                switch (index) {
                    ...
                    case clSieveSize:
                        ...
                        if (gSieveSize >
                            CSieveOfEratosthenesL1Ext::MaxSieveSize) {
                            fprintf(
                                stderr,
                    ...
                    case clWeaveDepth:
                        ...
                        if (gWeaveDepth > CSieveOfEratosthenesL1Ext::MaxDepth) {
                            fprintf(
                                stderr,
                                "Weave depth limited by %u, you try launch with %u\n",
                                CSieveOfEratosthenesL1Ext::MaxDepth,
                                gWeaveDepth);
                            exit(1);
                        }
                        break;
    ...
    GetBlockTemplateContext ctx(
        log, gUrl, gUserName, gPassword, gWallet, 4, gThreadsNum, extraNonce);
    ctx.run();
...

    {
        ...
        printf(
            " ** xpmcpuminer started %s %s %s worker %i **\n",
            buffer,
            gUrl,
            gWallet,
            extraNonce);
