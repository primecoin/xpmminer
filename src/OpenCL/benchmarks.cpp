...
bool sieveResultsTest(
    ...
    *candidatesCount = 0;
    ...
    for (unsigned extNum = 0; extNum <= extensionsNum; extNum++) {
        const uint32_t* ptr =
            ((const uint32_t*)bytefield) + extNum * sieveWords;
        while (i < sieveWords) {
            ...
            for (unsigned j = 0; j < 32; j++, sieveWord >>= 1) {
                ...
                unsigned M =
                    8 * GPUSieveWindowSize * (i * 4 / GPUSieveWindowSize) +
                    (i * 4 + j / 8) % GPUSieveWindowSize +
                    (j & 0x7) * GPUSieveWindowSize;
                if (M == 0)
                    continue;
                ...
                if (type == PRIME_CHAIN_CUNNINGHAM1) {
                    if (!trialDivisionChainTest(
                        ...
                        fprintf(
                            stderr,
                            "multiplier %u can't pass trial division test\n",
                            candidateMultiplier);
                        return false;
                    }
...
void sieveBenchmark(
    ...
    std::unique_ptr<CSieveOfEratosthenesL1ExtPtr[]> sieves(
        new CSieveOfEratosthenesL1Ext*[groupsNum]);
    ...
    for (unsigned i = 0; i < groupsNum; i++) {
        ...
        if (!updateBlock(&header, blockHeaderHash, primeSource, testParams)) {
            fprintf(
                stderr,
                "sieveBenchmark error: cannot find block with correct SHA256 hash\n");
            exit(1);
        }

        fixedMultipliers[i] = primorial * blockHeaderHash;
        mpz_export(
            &fixedMultipliersBufferGPU[GPUMultiprecisionLimbs * i],
            &limbsNumber,
            -1,
            4,
            0,
            0,
            fixedMultipliers[i].get_mpz_t());
    }

    cl_mem fixedMultipliersDevPtr;
    if (clUpload<uint32_t>(
            device,
            fixedMultipliersBufferGPU,
            GPUMultiprecisionLimbs * groupsNum,
            &fixedMultipliersDevPtr,
            CL_MEM_READ_WRITE)) {
        fprintf(
            stderr,
            "sieveBenchmark error: cannot allocate memory for sieve bitfields\n");
        exit(1);
    }

    cl_kernel kernel = device.kernels[CLKernelSieveBenchmark];
    clSetKernelArg(
        kernel, 0, sizeof(fixedMultipliersDevPtr), &fixedMultipliersDevPtr);
    ...
    clSetKernelArg(
        kernel, 4, sizeof(device.primesDevPtr), &device.primesDevPtr);
    clSetKernelArg(
        kernel,
        5,
        sizeof(device.multipliers64DevPtr),
        &device.multipliers64DevPtr);
    clSetKernelArg(
        kernel, 6, sizeof(device.offsets64DevPtr), &device.offsets64DevPtr);

    size_t sieveBufferSize = GPUMaxSieveSize * (gExtensionsNum + 1) / 8;
    std::unique_ptr<uint8_t[]> cunningham1Bytefield(
        new uint8_t[sieveBufferSize]);
    std::unique_ptr<uint8_t[]> cunningham2Bytefield(
        new uint8_t[sieveBufferSize]);
    ...
    for (unsigned i = 2; i <= 16; i += 2) {
        ...
        unsigned realSieveSize = sieveSize +
            sieveSize / 2 * CSieveOfEratosthenesL1Ext::ExtensionsNum;
        ...
        for (unsigned pass = 0; pass < passCount; pass++) {
            ...
            if ((result = clEnqueueNDRangeKernel(
                     device.queue,
                     kernel,
                     1,
                     0,
        ...
        for (unsigned groupIdx = 0; groupIdx < groupsNum; groupIdx++) {
            ...
            while (
                sieves[groupIdx]->GetNextCandidateMultiplier(multiplier, type))
        ...
        for (unsigned groupIdx = 0; groupIdx < groupsNum; groupIdx++) {
            ...
            results[0] = clEnqueueReadBuffer(
                device.queue,
                device.cunningham1,
                CL_TRUE,
                groupIdx * sieveBufferSize,
                sieveBufferSize,
            ...
            results[2] = clEnqueueReadBuffer(
                ...
                NULL,
                &events[2]);
            if (results[0] != CL_SUCCESS || results[1] != CL_SUCCESS ||
                results[2] != CL_SUCCESS ||
                clWaitForEvents(3, events) != CL_SUCCESS) {
                fprintf(
                    stderr,
                    " * Error: cannot copy memory from device to host\n");
                exit(1);
            }
            ...
            if (!sieveResultsTest(
                ...
                fprintf(
                    stderr,
                    " * Error: OpenCL device produces wrong results! "
...
void gpuMinerBenchmark(
    ...
    std::unique_ptr<GPUNonceAndHash[]> nonceAndHash(
        new GPUNonceAndHash[groupsNum]);
    std::unique_ptr<FermatQueue[]> queue(new FermatQueue[groupsNum]);
    std::unique_ptr<FermatTestResults[]> results_(
        new FermatTestResults[groupsNum]);
    memset(nonceAndHash.get(), 0, sizeof(GPUNonceAndHash) * groupsNum);
    memset(queue.get(), 0, sizeof(FermatQueue) * groupsNum);

    generateRandomHeader(&header, difficulty);

    OpenCLNewBlockPrepare(
        device, groupsNum, header, nonceAndHash.get(), queue.get());
    ...
    for (unsigned round = 0; round < roundsNum; round++) {
            ...
            for (unsigned groupIdx = 0; groupIdx < groupsNum; groupIdx++) {
                ...
                for (unsigned mIdx = 0; mIdx < results.size; mIdx++) {
                    ...
                    mpz_import(
                        ...
                        0,
                        hashData);
                    if (!ProbablePrimalityTestWithTrialDivisionFast(
                            blockHeaderHash, 1000, primeSource, testParams)) {
                        fprintf(
                            stderr,
                            " * Error: invalid nonce found on OpenCL device\n");

                        for (unsigned i = 0; i < 1000; i++) {
                            if (blockHeaderHash % primeSource.prime(i) == 0) {
                                fprintf(
                                    stderr,
                                    "   * block header hash not prime (can be divided by %u)\n",
                                    primeSource.prime(i));
                                return;
                            }
                        }
                        ...
                        unsigned nLength;
                        if (!FermatProbablePrimalityTestFast(
                                blockHeaderHash, nLength, testParams, true))
...

                    {
                        ...
                        if (type == PRIME_CHAIN_CUNNINGHAM1) {
                            if (!trialDivisionChainTest(
                                ...
                                fprintf(
                                    stderr,
                                    " * Error: C1-multiplier %u can't pass trial division test\n",
                                    triedMultiplier);
                                continue;
                            }
                        } else if (type == PRIME_CHAIN_CUNNINGHAM2) {
                            if (!trialDivisionChainTest(
                                ...
                                fprintf(
                                    stderr,
                                    " * Error: C2-multiplier %u can't pass trial division test\n",
                                    triedMultiplier);
                                continue;
                            }
                        } else {
                            ...
                            if (!trialDivisionChainTest(
                                ...
                                fprintf(
                                    stderr,
                                    " * Error: BT-multiplier %u can't pass trial division test\n",
                                    triedMultiplier);
                                continue;
                            }
                        }
                    ...
                    if (results.resultChainLength[mIdx] !=
                        chainLengthFromBits(testParams.chainLength)) {
                        fprintf(
                            stderr,
                            "multiplier [%u]%u difference, CPU: %u, GPU: %u\n",
                            nonce,
                            triedMultiplier,
                            (unsigned)chainLengthFromBits(
                                testParams.chainLength),
        ...
        printf(
            "  * [%u] round time: %.3lf milliseconds, speed: %.3lf M\n",
            round,
            tm / 1000.0,
...
void multiplyBenchmark(
    ...
    if (mulOperandSize == 128 / 32) {
        ...
        fprintf(
            stderr,
            "Can't multiply %u-size operands on OpenCL device\n",
            mulOperandSize * 32);
        return;
    }
    ...
    for (unsigned i = 0; i < elementsNum; i++) {
        ...
        mpz_import(
            ...
            &m2[i * mulOperandSize]);
        mpz_import(
            cpuResult[i].get_mpz_t(),
            mulOperandSize * 2,
            -1,
            4,
            0,
            0,
            &mR[i * mulOperandSize * 2]);
    }
    ...
    printf(
        "Multiply %u bits CPU time: %.3lf, GPU time: %.3lf (%.3lf times faster)\n",
        mulOperandSize * 32,
        cpuTime,
        gpuTime,
        cpuTime / gpuTime);
...

{
    ...
    for (unsigned i = 0; i < elementsNum; i++) {
        for (unsigned j = 0; j < dividendOperandSize; j++)
            dividends[i * dividendOperandSize + j] =
                (j == (dividendOperandSize - 4)) ? 1 + rand() % 2 : 0;
        switch (i % 4) {
            case 0:
                for (unsigned j = 0; j < divisorOperandSize; j++)
                    divisors[i * divisorOperandSize + j] = rand32()
                        << (rand() % 4);
                break;
            case 1:
                for (unsigned j = 0; j < divisorOperandSize; j++)
                    divisors[i * divisorOperandSize + j] =
                        (j >= (divisorOperandSize - 1))
                        ? 0
                        : rand32() << (rand() % 4);
                break;
            case 2:
    ...
    clUpload(
        ...
        CL_MEM_READ_ONLY);
    clAlloc<uint32_t>(
        device, divisorLimbsNum, &gpuModulosBuffer, CL_MEM_READ_WRITE);
    if (dividendOperandSize == 384 / 32 && divisorOperandSize == 256 / 32) {
        kernel = device.kernels[CLKernelModulo384to256test];
    } else if (
        dividendOperandSize == 512 / 32 && divisorOperandSize == 384 / 32) {
        kernel = device.kernels[CLKernelModulo512to384test];
    } else if (
        dividendOperandSize == 640 / 32 && divisorOperandSize == 512 / 32) {
        kernel = device.kernels[CLKernelModulo640to512test];
    } else {
        fprintf(
            stderr,
            "Can't divide %ubits to %ubits in OpenCL (no such implementation)\n",
    ...
    for (unsigned i = 0; i < elementsNum; i++) {
        mpz_import(
            cpuDividendsBuffer[i].get_mpz_t(),
            dividendOperandSize,
            -1,
            4,
            0,
            0,
            &dividends[i * dividendOperandSize]);
        mpz_import(
            cpuDivisorsBuffer[i].get_mpz_t(),
            divisorOperandSize,
            -1,
            4,
            0,
            0,
            &divisors[i * divisorOperandSize]);
        mpz_import(
            cpuModulosBuffer[i].get_mpz_t(),
            divisorOperandSize,
            -1,
            4,
            0,
            0,
            &cpuModulos[i * divisorOperandSize]);
    }
    ...
    for (unsigned i = 0; i < elementsNum; i++) {
        ...
        mpz_export(
            ...
            cpuModulosBuffer[i].get_mpz_t());
        if (memcmp(
                &gpuModulos[i * divisorOperandSize],
                &cpuModulos[i * divisorOperandSize],
                4 * divisorOperandSize) != 0) {
            fprintf(stderr, "element index: %u\n", i);
            fprintf(stderr, "gmp: ");
            for (unsigned j = 0; j < divisorOperandSize; j++)
                fprintf(
                    stderr, "%08X ", cpuModulos[i * dividendOperandSize + j]);
            fprintf(stderr, "\ngpu: ");
            for (unsigned j = 0; j < divisorOperandSize; j++)
                fprintf(
                    stderr, "%08X ", gpuModulos[i * dividendOperandSize + j]);
            fprintf(stderr, "\n");
            fprintf(stderr, "results differ!\n");
            break;
        }
    ...
    printf(
        "Modulo (%u %% %u) CPU time: %.3lf, GPU time: %.3lf (%.3lf times faster)\n",
        dividendOperandSize * 32,
        divisorOperandSize * 32,
        cpuTime,
        gpuTime,
...
void fermatTestBenchmark(
    ...
    for (unsigned i = 0; i < elementsNum; i++) {
        for (unsigned j = 0; j < operandSize; j++)
            numbers[i * operandSize + j] =
                (j == operandSize - 1) ? (1 << (i % 32)) : rand32();
    ...
    clUpload(
        ...
        CL_MEM_READ_ONLY);
    clAlloc<uint32_t>(
        device, numberLimbsNum, &gpuResultsBuffer, CL_MEM_READ_WRITE);
    if (operandSize == 256 / 32) {
        ...
        fprintf(
            stderr,
            "Can't do Fermat test on %ubit operand\n",
            operandSize * 32);
        return;
    }
    ...
    double gpuTime = usDiff(gpuBegin, gpuEnd) / 1000000.0;
    printf(
        "Fermat test %u bits CPU time: %.3lf, GPU time: %.3lf (%.3lf times faster)\n",
        operandSize * 32,
        cpuTime,
        gpuTime,
        cpuTime / gpuTime);
