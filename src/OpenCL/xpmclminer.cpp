...
void copyMultiplierToBlock(
    PrimecoinBlockHeader& header,
    const mpz_class& primorial,
    unsigned M) {
    uint8_t buffer[256];
    BIGNUM* xxx = 0;
...
void* mine(void* arg) {
    ...
    if (int error = OpenCLKernelsPrepare(
            *ctx->platform,
            device,
            primeSource,
            primorial,
    ...
    while (1) {
        bool hasChanged;
        while (
            !(workTemplate = ctx->gbp->get(
                  ctx->threadIdx, workTemplate, &dataId, &hasChanged)))
            usleep(1000);
        ...
        if (hasChanged) {
            ...
            memset(
                nonceAndHash.get(),
                0,
                sizeof(GPUNonceAndHash) * device.groupsNum);
        ...
        if (OpenCLMiningRound(device, device.groupsNum, results_.get())) {
            for (unsigned groupIdx = 0; groupIdx < device.groupsNum;
                 groupIdx++) {
                ...
                for (unsigned i = 0; i < results.size; i++) {
                    ...
                    if (chainLength >= chainLengthFromBits(work.bits)) {
                        ...
                        mpz_class chainOrigin =
                            fixedMultiplier * triedMultiplier;
                        ...
                        std::string chainName = GetPrimeChainName(
                            testParams.candidateType, testParams.chainLength);
                        ...
                        copyMultiplierToBlock(
                            work, primorial, results.resultMultipliers[i]);
...
void printHelpMessage() {
    ...
    printf(
        "  --sieve-size <number>: Eratosthenes sieve size, must be multiply of sieving window size(%u) (default: %u)\n",
        GPUSieveWindowSize,
        gSieveSize);
    printf(
        ...
        gWeaveDepth);
    printf(
        "  --opencl-platform: OpenCL platform name, (default: Advanced Micro Devices, Inc.)\n");
    printf("  --use-cpu: run code on CPU (for debugging purposes)\n");
    printf(
        "  --disable-opt: disables OpenCL code optimizations (for debugging purposes)\n");
    printf(
...
int main(int argc, char** argv) {
                ...
                switch (index) {
                    ...
                    case clPrimorial:
                        ...
                        if (gPrimorial > 19) {
                            fprintf(
                                stderr,
                                "OpenCL implementation does not support more than 384-bit "
                    ...
                    case clSieveSize:
                        ...
                        if (gSieveSize % GPUSieveWindowSize != 0) {
                            fprintf(
                                stderr,
                                "Sieve size must be a multiple of %u\n",
                                GPUSieveWindowSize);
                            exit(1);
                        }
                        if (gSieveSize > GPUMaxSieveSize) {
                            fprintf(
                                stderr,
                                "Sieve size limited by %u, you try launch with %u\n",
                                GPUMaxSieveSize,
                                gSieveSize);
                            exit(1);
                        }
...

    {
        ...
        printf(
            " ** xpmclminer started %s %s %s worker %i **\n",
            buffer,
            gUrl,
            gWallet,
            extraNonce);
