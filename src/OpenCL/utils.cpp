...
static char* readFile(const char* name) {
    ...
    if (fread(kernelSource.get(), 1, sourceFileSize, sourceFile.get()) !=
        sourceFileSize)
...
int compileKernel(
    ...
    *context = clCreateContext(
        contextProperties, devicesNum, devices, 0, 0, &clResult);
    if (clResult != CL_SUCCESS || !*context)
        return logError(
            1, stderr, " * Error: can't create OpenCL context for this device");

    *program =
        clCreateProgramWithSource(*context, 1, &kernelSource, 0, &clResult);
    if (clResult != CL_SUCCESS || *program == 0)
        return logError(
            1, stderr, "Error: can't compile OpenCL kernel %s\n", kernelFile);

    if (clBuildProgram(*program, devicesNum, devices, cmdLine, 0, 0) !=
        CL_SUCCESS) {
        size_t logSize;
        clGetProgramBuildInfo(
            *program, devices[0], CL_PROGRAM_BUILD_LOG, 0, 0, &logSize);

        std::unique_ptr<char[]> log(new char[logSize]);
        clGetProgramBuildInfo(
            *program, devices[0], CL_PROGRAM_BUILD_LOG, logSize, log.get(), 0);
        fprintf(stderr, "%s\n", log.get());

        return logError(
            1, stderr, "Error: can't compile OpenCL kernel %s", kernelFile);
    }
...
int OpenCLInit(
    ...
    cl_platform_id targetPlatform = 0;
    {
        ...
        if (clGetPlatformIDs(numPlatforms, platforms.get(), 0) != CL_SUCCESS)
            return logError(
                1, stderr, "Error: no any OpenCL platforms found\n");
        ...
        for (cl_uint i = 0; i < numPlatforms; i++) {
            ...
            if (clGetPlatformInfo(
                    ...
                    0) != CL_SUCCESS)
                return logError(
                    1, stderr, "Error: can't enumerate OpenCL platforms\n");
    ...
    if (!targetPlatform)
        return logError(
            1, stderr, "Error: can't find plaform: %s\n", targetPlatformName);
    ...
    if (!ctx.devicesNum)
        return logError(
            1, stderr, "Error: can't find '%s' GPUs\n", targetPlatformName);
    ...
    if (clGetDeviceIDs(
            ...
            0) != CL_SUCCESS)
        return logError(
            1, stderr, "Error: can't query '%s' devices\n", targetPlatformName);

    cl_context context = 0;
    cl_program program = 0;
    if (!useSeparateContext) {
        int result = compileKernel(
            targetPlatform,
            ctx.devicesNum,
            devices.get(),
            kernelFile,
    ...
    for (cl_uint i = 0; i < ctx.devicesNum; i++) {
        ...
        clGetDeviceInfo(
            devices[i], CL_DEVICE_NAME, sizeof(deviceName), deviceName, 0);
        clGetDeviceInfo(
            devices[i],
        ...
        deviceCtx.queue =
            clCreateCommandQueue(deviceCtx.context, devices[i], 0, &clResult);
        if (clResult != CL_SUCCESS || !deviceCtx.queue)
            return logError(
                1,
                stderr,
                " * Error: can't create command queue for this device\n");
        ...
        if (GPA_OpenContext(deviceCtx.queue) != GPA_STATUS_OK ||
            ...
            return logError(
                1,
                stderr,
                "Error: GPA_OpenContext/GPA_EnableAllCounters failed\n");
#endif
    }
...
int OpenCLKernelsPrepare(
    ...
    uint32_t primorialBuffer[8];
    {
        ...
        mpz_export(
            primorialBuffer, &limbsNumber, -1, 4, 0, 0, primorial.get_mpz_t());
    }

    device.kernels.reset(new cl_kernel[CLKernelsNum]);
    for (size_t i = 0; i < CLKernelsNum; i++) {
        device.kernels[i] =
            clCreateKernel(device.program, gOpenCLKernelNames[i], &clResult);
        if (clResult != CL_SUCCESS)
            return logError(
                1,
                stderr,
                "Error: can't find function %s in kernel\n",
    ...
    if (clUpload<uint32_t>(
            device,
            primeSource.primesPtr(),
            weaveDepth,
            &device.primesDevPtr,
            CL_MEM_READ_ONLY) ||
        clUpload<uint64_t>(
            device,
            primeSource.multiplier64Ptr(),
            weaveDepth,
            &device.multipliers64DevPtr,
            CL_MEM_READ_ONLY) ||
        clUpload<uint32_t>(
            device,
            primeSource.offsets64Ptr(),
            weaveDepth,
            &device.offsets64DevPtr,
            CL_MEM_READ_ONLY) ||
        clAlloc<uint8_t>(
            device,
            device.groupsNum * sieveBufferSize,
            &device.cunningham1,
            CL_MEM_READ_WRITE) != 0 ||
        clAlloc<uint8_t>(
            device,
        ...
        clAlloc<uint8_t>(
            ...
            &device.bitwin,
            CL_MEM_READ_WRITE) != 0 ||
        clAlloc<PrimecoinBlockHeader>(
            device, 1, &device.blockHeaderDevPtr, CL_MEM_READ_ONLY) != 0 ||
        clUpload<uint32_t>(
            device,
            primorialBuffer,
            8,
            &device.primorialDevPtr,
            CL_MEM_READ_ONLY) ||
        clAlloc<GPUNonceAndHash>(
            device,
            device.groupsNum,
            &device.nonceAndHashDevPtr,
            CL_MEM_READ_WRITE) != 0 ||
        clAlloc<FermatQueue>(
            device,
            device.groupsNum,
            &device.c1QueueDevPtr,
            CL_MEM_READ_WRITE) != 0 ||
        clAlloc<FermatQueue>(
            device,
            device.groupsNum,
            &device.c2QueueDevPtr,
            CL_MEM_READ_WRITE) != 0 ||
        clAlloc<FermatQueue>(
            device,
            device.groupsNum,
            &device.btQueueDevPtr,
            CL_MEM_READ_WRITE) != 0 ||
        clAlloc<FermatTestResults>(
            device,
            device.groupsNum,
            &device.resultsDevPtr,
            CL_MEM_READ_WRITE) != 0)
        return 1;

    clSetKernelArg(
        device.kernels[CLKernelSearchNonce],
        0,
        sizeof(device.blockHeaderDevPtr),
        &device.blockHeaderDevPtr);
    clSetKernelArg(
        device.kernels[CLKernelSearchNonce],
        1,
        sizeof(device.nonceAndHashDevPtr),
        &device.nonceAndHashDevPtr);
    clSetKernelArg(
        device.kernels[CLKernelSearchNonce],
        2,
        sizeof(device.primesDevPtr),
        &device.primesDevPtr);
    clSetKernelArg(
        device.kernels[CLKernelSearchNonce],
        3,
    ...
    clSetKernelArg(
        device.kernels[CLKernelSearchNonce],
        4,
        sizeof(device.offsets64DevPtr),
        &device.offsets64DevPtr);

    clSetKernelArg(
        device.kernels[CLKernelSieve],
        0,
        sizeof(device.cunningham1),
        &device.cunningham1);
    clSetKernelArg(
        device.kernels[CLKernelSieve],
        1,
        sizeof(device.cunningham2),
        &device.cunningham2);
    clSetKernelArg(
        device.kernels[CLKernelSieve],
        2,
        sizeof(device.bitwin),
        &device.bitwin);
    clSetKernelArg(
        device.kernels[CLKernelSieve],
        3,
        sizeof(device.primorialDevPtr),
        &device.primorialDevPtr);
    clSetKernelArg(
        device.kernels[CLKernelSieve],
        4,
    ...
    clSetKernelArg(
        device.kernels[CLFermatTestEnqueue],
        0,
        sizeof(device.nonceAndHashDevPtr),
        &device.nonceAndHashDevPtr);
    clSetKernelArg(
    ...
    clSetKernelArg(
        device.kernels[CLFermatTestEnqueue],
        5,
        sizeof(device.c1QueueDevPtr),
        &device.c1QueueDevPtr);
    clSetKernelArg(
        device.kernels[CLFermatTestEnqueue],
        6,
        sizeof(device.c2QueueDevPtr),
        &device.c2QueueDevPtr);
    clSetKernelArg(
        device.kernels[CLFermatTestEnqueue],
        7,
        sizeof(device.btQueueDevPtr),
        &device.btQueueDevPtr);
    clSetKernelArg(
        device.kernels[CLFermatTestEnqueue],
        8,
        sizeof(device.resultsDevPtr),
        &device.resultsDevPtr);

    clSetKernelArg(
        device.kernels[CLFermatTestEnqueueBt],
        0,
        sizeof(device.nonceAndHashDevPtr),
        &device.nonceAndHashDevPtr);
    clSetKernelArg(
...

{
    ...
    results[0] = clEnqueueWriteBuffer(
        device.queue,
        device.blockHeaderDevPtr,
        CL_TRUE,
        0,
        ...
        NULL,
        &events[0]);
    results[1] = clEnqueueWriteBuffer(
        device.queue,
        device.nonceAndHashDevPtr,
        CL_TRUE,
        0,
        sizeof(GPUNonceAndHash) * groupsNum,
        nonceZeroBuffer,
        0,
        NULL,
        &events[1]);
    results[2] = clEnqueueWriteBuffer(
        device.queue,
        device.c1QueueDevPtr,
        CL_TRUE,
        0,
        sizeof(FermatQueue) * groupsNum,
        queueZeroBuffer,
        0,
        NULL,
        &events[2]);
    results[3] = clEnqueueWriteBuffer(
        device.queue,
        device.c2QueueDevPtr,
        CL_TRUE,
        0,
        ...
        NULL,
        &events[3]);
    results[4] = clEnqueueWriteBuffer(
        device.queue,
        device.btQueueDevPtr,
        CL_TRUE,
        0,
    ...
    if (results[0] != CL_SUCCESS || results[1] != CL_SUCCESS ||
        results[2] != CL_SUCCESS || results[3] != CL_SUCCESS ||
        results[4] != CL_SUCCESS) {
        fprintf(stderr, " * Error: cannot enqueue write buffer operation\n");
        return false;
    }

    if (clWaitForEvents(sizeof(events) / sizeof(cl_event), events) !=
        CL_SUCCESS) {
...
bool OpenCLMiningRound(
    ...
    if ((result = clEnqueueReadBuffer(
             device.queue,
             device.resultsDevPtr,
             CL_TRUE,
             0,
             sizeof(FermatTestResults) * groupsNum,
             results,
             2,
             &events[2],
             &events[4])) != CL_SUCCESS) {
    ...
    for (gpa_uint32 index = 0; index < count; index++) {
        ...
        switch (type) {
            ...
            case GPA_TYPE_UINT32:
                if (GPA_GetSampleUInt32(
                        sessionId, sampleId, index, &ResultUInt32) ==
                    GPA_STATUS_OK)
                    snprintf(
                        counterText, sizeof(counterText), "%u", ResultUInt32);
                break;
            case GPA_TYPE_INT64:
            case GPA_TYPE_UINT64:
                if (GPA_GetSampleUInt64(
                        sessionId, sampleId, index, &ResultUInt64) ==
                    GPA_STATUS_OK)
                    snprintf(
                        counterText, sizeof(counterText), "%llu", ResultUInt64);
                break;
            case GPA_TYPE_FLOAT32:
                if (GPA_GetSampleFloat32(
                        sessionId, sampleId, index, &ResultFloat32) ==
                    GPA_STATUS_OK)
                    snprintf(
                        counterText,
                        sizeof(counterText),
            ...
            case GPA_TYPE_FLOAT64:
                if (GPA_GetSampleFloat64(
                        sessionId, sampleId, index, &ResultFloat64) ==
                    GPA_STATUS_OK)
                    snprintf(
                        counterText,
                        sizeof(counterText),
