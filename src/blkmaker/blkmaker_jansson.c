...
static const char* parse_txn(struct blktxn_t* txn, json_t* txnj) {
    json_t* vv;
...
static void my_flip(void* data, size_t datasz) {
...
const char* blktmpl_add_jansson(
    blktemplate_t* tmpl,
    const json_t* json,
    time_t time_rcvd) {
    if (tmpl->version)
    ...
    if (tmpl->version > 2 || (tmpl->version == 2 && !tmpl->height)) {
        ...
        else if (!(tmpl->mutations & BMM_VERFORCE))
...
static char varintEncode(unsigned char* out, uint64_t n) {
    if (n < 0xfd) {
        out[0] = n;
        return 1;
    }
...
static json_t* _blkmk_submit_jansson(
    blktemplate_t* tmpl,
...
json_t* blkmk_submit_jansson(
    ...
    const unsigned char* extra,
    size_t extrasz) {
    return _blkmk_submit_jansson(
        tmpl, data, dataid, nonce, false, extra, extrasz);
}

json_t* blkmk_submit_foreign_jansson(
    blktemplate_t* tmpl,
    const unsigned char* data,
    unsigned int dataid,
    blknonce_t nonce) {
