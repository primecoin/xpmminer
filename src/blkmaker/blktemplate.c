...
gbt_capabilities_t blktmpl_addcaps(const blktemplate_t* tmpl) {
    ...
    if (tmpl->version)
        return 0;
    return GBT_CBTXN | GBT_WORKID | BMM_TIMEINC | BMM_CBAPPEND | BMM_VERFORCE |
        BMM_VERDROP | BMAb_COINBASE | BMAb_TRUNCATE;
}
...
void blktxn_copy(struct blktxn_t* out, struct blktxn_t* in) {
    ...
    if (in->depends) {
        ...
        memcpy(
            out->depends, in->depends, sizeof(unsigned long) * in->dependcount);
    }
}
