...
int main(int argc, char** argv) {
    ...
    GetBlockTemplateContext* getblock = new GetBlockTemplateContext(
        0, gUrl, gUserName, gPassword, gWallet, 4, gThreadsNum, extraNonce);
    getblock->run();
    blktemplate_t* workTemplate = 0;
    unsigned int dataId;
    bool hasChanged;
    ...
    for (unsigned i = 0; i < 0; i++) {
        cudaRunBenchmarks(
            gpus[i].context, gpus[i].device, modules[i], depth, clKernelLSize);
    }
