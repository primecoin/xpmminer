   ...
   public:
    ...
    ~cudaBuffer() {
        ...
        if (_deviceData) {
            ...
            if (result != CUDA_SUCCESS) {
                ...
                LOG_F(
                    ERROR,
                    "CUDA memory free failed with error %s code: %i\n",
                    msg,
                    static_cast<int>(result));
            }
        }
    ...
    CUresult copyToDevice(CUstream stream) {
        return cuMemcpyHtoDAsync(
            _deviceData, _hostData, sizeof(T) * _size, stream);
    }
    ...
    CUresult copyToDevice(T* hostData, CUstream stream) {
        return cuMemcpyHtoDAsync(
            _deviceData, hostData, sizeof(T) * _size, stream);
    }
    ...
    CUresult copyToHost(CUstream stream) {
        return cuMemcpyDtoHAsync(
            _hostData, _deviceData, sizeof(T) * _size, stream);
    }
