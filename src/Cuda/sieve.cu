...
__constant__ uint32_t
    nps_all[] = {4, 4, 5, 6, 7, 7, 7, 9}; // 1024 threads per block (default)
#elif (LSIZELOG2 == 9)
...
#pragma unroll
        ...
        if (mask != 0xFFFFFFFF) {
            ...
            unsigned multiplier = bit + id * 32 +
                SIZE * 32 * STRIPES /
                    2; // mad24(id, 32u, (unsigned)bit) + SIZE*32*STRIPES/2;
            unsigned maxSize =
                hashSize + (32 - __clz(multiplier)) + start + depth;
            const uint32_t addr =
                atomicAdd(&fcount[(maxSize <= 320) ? 0 : 1], 1);
...
#pragma unroll
        ...
        if (mask != 0xFFFFFFFF) {
            ...
            unsigned multiplier = bit + id * 32 +
                SIZE * 32 * STRIPES /
                    2; // mad24(id, 32u, (unsigned)bit) + SIZE*32*STRIPES/2;
            unsigned maxSize = hashSize + (32 - __clz(multiplier)) + start +
                (depth / 2) + (depth & 1);
            const uint32_t addr =
                atomicAdd(&fcount[(maxSize <= 320) ? 0 : 1], 1);
            fermat_t* found = (maxSize <= 320) ? found320 : found352;
