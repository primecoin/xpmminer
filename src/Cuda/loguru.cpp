...
void file_log(void* user_data, const Message& message) {
    ...
    if (duration_cast<seconds>(
            steady_clock::now() - file_abs->last_check_time) > check_interval) {
...
void file_reopen(void* user_data) {
    ...
    if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 ||
        ...
        if (!file_abs->fp) {
            LOG_F(
                INFO,
                "Reopening file '%s' due to previous error",
                file_abs->path);
        } else if (ret < 0) {
            const auto why = errno_as_text();
            LOG_F(
                INFO,
                "Reopening file '%s' due to '%s'",
                file_abs->path,
                why.c_str());
        } else {
            LOG_F(
                INFO,
                "Reopening file '%s' due to file changed",
                file_abs->path);
        }
        // try reopen current file.
        if (!create_directories(file_abs->path)) {
            LOG_F(
                ERROR, "Failed to create directories to '%s'", file_abs->path);
        }
        ...
        if (strncmp(cmd, verbosity_flag, arg_len) == 0 &&
            ...
            if (value_str[0] == '\0') {
                ...
                CHECK_LT_F(
                    arg_it,
                    argc,
                    "Missing verbosiy level after %s",
                    verbosity_flag);
...
bool add_file(const char* path_in, FileMode mode, Verbosity verbosity) {
    ...
    LOG_F(
        INFO,
        "Logging to '%s', mode: '%s', verbosity: %d",
        path,
        mode_str,
        verbosity);
    return true;
