...
static inline unsigned calculateOffset(
    unsigned lowIdx,
...
void CSieveOfEratosthenesL1Ext::reset(
    unsigned sieveSize,
    ...
    for (unsigned i = 1; i <= ExtensionsNum; i++) {
        memset(
            cunningham1Bitfield + i * sieveWords + sieveWords / 2,
            0,
            (sieveWords / 2) * 8);
        memset(
            cunningham2Bitfield + i * sieveWords + sieveWords / 2,
            0,
            (sieveWords / 2) * 8);
        memset(
            bitwinBitfield + i * sieveWords + sieveWords / 2,
            0,
            (sieveWords / 2) * 8);
    }
}
...
unsigned int CSieveOfEratosthenesL1Ext::GetCandidateCount() {
    ...
    for (unsigned i = 0; i < sieveWords; i++) {
        uint64_t value =
            ~(cunningham1Bitfield[i] & cunningham2Bitfield[i] &
              bitwinBitfield[i]);
        if (!value)
            continue;
    ...
    // Extensions
    uint64_t* extCunningham1 =
        cunningham1Bitfield + sieveWords + sieveWords / 2;
    uint64_t* extCunningham2 =
        cunningham2Bitfield + sieveWords + sieveWords / 2;
...
bool CSieveOfEratosthenesL1Ext::GetNextCandidateMultiplier(
    unsigned& nVariableMultiplier,
        ...
        if (word) {
            for (; bitOffset < 64; bitOffset++, word >>= 1) {
                if (word & 0x1) {
                    ...
                    nVariableMultiplier =
                        (wordIdx * 64 + bitOffset) * (1 << extensionNum);
                    bitOffset++;
                    word >>= 1;
                    return true;
                }
...
static uint32_t longModuloByMul(
    uint32_t* number,
    uint32_t divider,
    uint64_t multiplier,
    unsigned shift) {
...
void CSieveOfEratosthenesL1Ext::Weave() {
    ...
    for (unsigned i = 0; i < depth; i++) {
        ...
        unsigned qtmp =
            (a_mod_xy * _primeSource.multiplier(i)) >> _primeSource.offset(i);
        unsigned a_mod_xy_mod_x = a_mod_xy - (qtmp * currentPrime);
        unsigned inverseModulo =
            a_mod_xy_mod_x ? intInvert(a_mod_xy_mod_x, currentPrime) : 0;
        ...
        if (inverseModulo) {
            for (unsigned layer = 0; layer < chainLength; layer++) {
                ...
                cunningham2Multipliers[layersNum * i + layer] =
                    currentPrime - inverseModulo;
                inverseModulo = (inverseModulo & 0x1)
                    ? (inverseModulo + currentPrime) / 2
                    : inverseModulo / 2;
            }
            ...
            for (unsigned layer = chainLength; layer < layersNum; layer++) {
                ...
                qtmp =
                    ((uint64_t)extensionLowIdx * _primeSource.multiplier(i)) >>
                    _primeSource.offset(i);
                unsigned currentPrimeMod =
                    extensionLowIdx - (qtmp * currentPrime);

                cunningham1Multipliers[layersNum * i + layer] = calculateOffset(
                    extensionLowIdx,
                    currentPrime,
                    currentPrimeMod,
                    inverseModulo);
                cunningham2Multipliers[layersNum * i + layer] = calculateOffset(
                    extensionLowIdx,
                    currentPrime,
                    currentPrimeMod,
                    currentPrime - inverseModulo);
    ...
    for (unsigned round = 0; round < roundsNum; round++) {
        ...
        for (unsigned layer = 0; layer < layersNum; layer++) {
            ...
            for (unsigned primeIdx = 0; primeIdx < depth; primeIdx++) {
                ...
                cunningham1Multipliers[layersNum * primeIdx + layer] =
                    offset + lowIdx;

                offset = cunningham2Multipliers[layersNum * primeIdx + layer] -
                    lowIdx;
                while (offset < L1CacheSize) {
                    setValue(local.cunningham2, offset);
                    offset += currentPrime;
                }
                cunningham2Multipliers[layersNum * primeIdx + layer] =
                    offset + lowIdx;
            }
            ...
            for (unsigned extNum =
                 ...
                 extNum++) {
                uint64_t* extCunningham1 =
                    cunningham1Bitfield + extNum * sieveWords + lowIdx / 64;
                uint64_t* extCunningham2 =
                    cunningham2Bitfield + extNum * sieveWords + lowIdx / 64;
                uint64_t* extBitwin =
                    bitwinBitfield + extNum * sieveWords + lowIdx / 64;
                ...
                if (layer - extNum < chainLength / 2) {
                    for (unsigned i = 0; i < L1CacheSize / 64; i++) {
                        ...
                        extBitwin[i] |=
                            local.cunningham1[i] | local.cunningham2[i];
                    }
    ...
    while (GetNextCandidateMultiplier(multiplier, type)) {
        ...
        if (type == PRIME_CHAIN_CUNNINGHAM1) {
            if (!trialDivisionChainTest(
                    _primeSource, chainOrigin, true, chainLength, depth))
                return false;
        } else if (type == PRIME_CHAIN_CUNNINGHAM2) {
            if (!trialDivisionChainTest(
                    _primeSource, chainOrigin, false, chainLength, depth))
                return false;
        } else {
