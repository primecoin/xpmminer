...
size_t GetBlockTemplateContext::curlWriteCallback(
    void* ptr,
    size_t size,
    size_t nmemb,
...
void GetBlockTemplateContext::updateWork() {
    ...
    json_t* je;
    if (!response ||
        ((je = json_object_get(response, "error")) && !json_is_null(je))) {
    ...
    for (unsigned i = 0; i < _blocksNum; i++) {
        ...
        uint64_t num = blkmk_init_generation2(
            _blocks[i],
        ...
        ssize_t coinbaseAppendResult = blkmk_append_coinbase_safe(
            _blocks[i], &extraNonce, sizeof(extraNonce));
        if (coinbaseAppendResult < 0) {
            logFormattedWrite(
                _log,
                "cannot add extra nonce (error %i)",
                (int)coinbaseAppendResult);
            return;
        }

        size_t dataSize = blkmk_get_data(
            _blocks[i], data, sizeof(data), time(0), NULL, &_dataId);
        if (!(dataSize >= 76 && dataSize <= sizeof(data))) {
            logFormattedWrite(
                _log,
                "getblocktemplate response decoding error (invalid size)");
            return;
        }
    }
...
GetBlockTemplateContext::GetBlockTemplateContext(
    void* log,
    const char* url,
    const char* user,
    const char* password,
...
blktemplate_t* GetBlockTemplateContext::get(
    unsigned blockIdx,
    blktemplate_t* old,
    unsigned* dataId,
    bool* hasChanged) {
...
size_t SubmitContext::curlWriteCallback(
    void* ptr,
    size_t size,
    size_t nmemb,
    SubmitContext* ctx) {
...
void SubmitContext::submitBlock(
    blktemplate_t* blockTemplate,
    const PrimecoinBlockHeader& header,
    unsigned dataId) {
    json_t* jsonBlock = blkmk_submit_jansson(
        blockTemplate,
    ...
    if (curl_easy_perform(curl) != CURLE_OK) {
        ...
        if (!response ||
            ((je = json_object_get(response, "error")) && !json_is_null(je)))
            fprintf(
                stderr, "\e[1;31m[JRPC ERROR] %s\e[0m\n", _response.c_str());
        else if ((je = json_object_get(response, "result")) && json_is_null(je))
            fprintf(
                stdout, "\e[1;32m[RPC SUBMIT] %s\e[0m\n", _response.c_str());
        else
