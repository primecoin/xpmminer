...
uint32_t bitsFromDifficulty(double difficulty) {
    ...
    uint32_t fractionalPart = (uint32_t)((difficulty - (double)chainLength) *
                                         (1U << DifficultyFractionalBits));
...
// "Magic number" search function (for replace division to multiplication &
// shift)
static bool findMultiplierForConstantDivision(
    unsigned maxDividendBits,
    ...
    for (unsigned i = 0; i < 2 * maxDividendBits + 1; i++) {
        ...
        if (powOf2 > nc * (divisor - 1 - ((powOf2 - 1) % divisor))) {
            *multiplier =
                (powOf2 + divisor - 1 - ((powOf2 - 1) % divisor)) / divisor;
            *offset = i;
            return true;
        }
    }
...
void generateMultipliers(
    ...
    for (unsigned i = 0; i < multipliersNum; i++) {
        findMultiplierForConstantDivision(
            31, primes[i], &inversedMultiplier, &offset);
        if (offset < 32)
            findMultiplierForConstantDivision(
                32, primes[i], &inversedMultiplier, &offset);

        findMultiplierForConstantDivision(
            63, primes[i], &inversedMultiplier64, &offset64);
        if (offset64 < 64)
            findMultiplierForConstantDivision(
                64, primes[i], &inversedMultiplier64, &offset64);
        if (primeCombinedIdx <= i) {
            ...
            findMultiplierForConstantDivision(
                63, divisor, &inversedCombinedMultiplier, &combinedOffset);
            isNewMultiplier[i] = 1;
        } else {
            isNewMultiplier[i] = 0;
        }
        ...
        mpz_export(
            ...
            0,
            0,
            inversedMultiplier64.get_mpz_t());
        mpz_export(
            &combinedMultipliers[i],
            &size,
            -1,
            4,
            0,
            0,
            inversedCombinedMultiplier.get_mpz_t());
        offsets[i] = offset;
...
PrimeSource::PrimeSource(uint32_t primesNum, unsigned inversedMultipliersNum)
    ...
    generateMultipliers(
        ...
        _multipliers64,
        _offsets64,
        _combinedMultipliers,
        _combinedOffsets,
        _primes,
        primesNum,
        inversedMultipliersNum);
}
...
bool ProbablePrimalityTestWithTrialDivisionFast(
    ...
    unsigned nLength = 0;
    return FermatProbablePrimalityTestFast(
        candidate, nLength, testParams, true);
}
...
static bool ProbableCunninghamChainTestFast(
    const mpz_class& n,
    bool fSophieGermain,
    bool fFermatTest,
    ...
    if (!FermatProbablePrimalityTestFast(
            n, nProbableChainLength, testParams, true))
        return false;
    ...
    while (true) {
        ...
        if (fFermatTest) {
            if (!FermatProbablePrimalityTestFast(
                    N, nProbableChainLength, testParams))
                break;
        } else {
            if (!EulerLagrangeLifchitzPrimalityTestFast(
                    N, fSophieGermain, nProbableChainLength, testParams))
                break;
        }
    }
...
bool ProbablePrimeChainTestFast(
    ...
    if (nCandidateType == PRIME_CHAIN_CUNNINGHAM1) {
        ...
        ProbableCunninghamChainTestFast(
            mpzOriginMinusOne, true, false, nChainLength, testParams);
        ...
        if (ProbableCunninghamChainTestFast(
            ...
            ProbableCunninghamChainTestFast(
                ...
                false,
                false,
                nChainLengthCunningham2,
                testParams);
            // Figure out BiTwin Chain length
            // BiTwin Chain allows a single prime at the end for odd length
            // chain
            nChainLength = (chainLengthFromBits(nChainLengthCunningham1) >
                            chainLengthFromBits(nChainLengthCunningham2))
                ? (nChainLengthCunningham2 +
                   bitsFromChainLengthWithoutFractional(
                       chainLengthFromBits(nChainLengthCunningham2) + 1))
                : (nChainLengthCunningham1 +
                   bitsFromChainLengthWithoutFractional(
                       chainLengthFromBits(nChainLengthCunningham1)));
        }
    }
...
std::string TargetToString(unsigned int nBits) {
    ...
    unsigned int currentlength =
        (nBits & DifficultyChainLengthMask) >> DifficultyFractionalBits;
    std::snprintf(
        buffer,
        sizeof(buffer),
        "%02x.%06x",
        currentlength,
        TargetGetFractional(nBits));
    return std::string(buffer);
}

std::string GetPrimeChainName(
    ...
    std::snprintf(
        ...
        sizeof(buffer),
        "%s%s",
        strLabels[std::min(nChainType, 4u)].c_str(),
        TargetToString(nChainLength).c_str());
    return std::string(buffer);
}

bool MineProbablePrimeChainFast(
    PrimecoinBlockHeader& header,
    ...
    sieve->reset(
        gSieveSize,
        chainLengthFromBits(header.bits),
        gWeaveDepth,
        hashMultiplier);
    sieve->Weave();
    ...
    while (true) {
        nTests++;
        if (!sieve->GetNextCandidateMultiplier(
                nTriedMultiplier, nCandidateType)) {
        ...
        if (ProbablePrimeChainTestFast(bnChainOrigin, testParams)) {
            ...
            std::reverse_copy(
                buffer + 4, buffer + 4 + buffer[3], header.multiplier + 1);
            fprintf(
                stderr,
                "targetMultiplier=%u\n",
                targetMultiplier.get_str().c_str());
            std::string chainName =
                GetPrimeChainName(nCandidateType, nChainLength);
