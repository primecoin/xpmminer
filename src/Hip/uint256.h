   ...
   public:
    ...
    std::string GetHex() const {
        ...
        for (unsigned int i = 0; i < sizeof(pn); i++)
            sprintf(
                psz + i * 2, "%02x", ((unsigned char*)pn)[sizeof(pn) - i - 1]);
    ...
    //    nVersion=PROTOCOL_VERSION) const
    ...
    template <typename Stream>
    //    void Unserialize(Stream& s, int nType=0, int
...
class uint256 : public base_uint256 {
   public:
    ...
    friend inline const uint256 operator<<(
        const uint256& a,
        unsigned int shift);
    friend inline const uint256 operator>>(
        const uint256& a,
