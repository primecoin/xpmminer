...
bool PrimeMiner::Initialize(hipCtx_t context, hipDevice_t device, hipModule_t module)
{
  _context = context;
                                ...
                                unsigned sievePerRound)
{
  // fermat dispatch
  {
    ...
    if(left > 0){
      hipMemcpyDtoDAsync(fermat.buffer[ridx].info._deviceData + count*sizeof(fermat_t),
