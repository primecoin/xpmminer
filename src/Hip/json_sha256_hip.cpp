...
__device__ void sha256_from_midstate(uint32_t* hash_out, uint32_t* midstate,
  ...
  // Append length in bits (this is partial hash, so we track from midstate)
  // For our use case, length should account for full message
  // This is a simplification - in real impl track total message length
  uint64_t bitlen = (128 + len) * 8; // 128 bytes before midstate + remaining
  for (int i = 7; i >= 0; i--) {
...
extern "C" __global__ void jsonHashMod(
  ...
  if (state[7] & (1u << 31)) {
    ...
    for (unsigned i = 0; i < 8; i++)
      prod13l = (prod13l * (primorialBitField & (1u << i) ? gPrimes[i] : 1u));
    prod13l *= (primorialBitField & (1u << 8) ? gPrimes[8] : 1u);

    uint64_t prod13 = prod13l;
    for (unsigned i = 9; i < 14; i++)
      prod13 *= (primorialBitField & (1u << i) ? gPrimes[i] : 1u);

    uint64_t prod14 = prod13 * (primorialBitField & (1u << 14) ? gPrimes[14] : 1u);
    uint64_t prod15 = prod14 * (primorialBitField & (1u << 15) ? gPrimes[15] : 1u);
