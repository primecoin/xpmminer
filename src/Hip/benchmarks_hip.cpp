...
// AMD RDNA optimization: Use 256 threads per block (8 wavefronts on wave32
// GPUs) This matches the LSIZE used in sieve kernels for consistency
        ...
        for (unsigned divIdx = 0; divIdx < depth; divIdx += 16) {
            if (mpz_tdiv_ui(N.get_mpz_t(), primes[divIdx]) == 0) {
                if (print)
                    LOG_F(
                        ERROR,
                        "Invalid number found; chain position is %u, divisor is %u type is %u",
                        i + 1,
                        primes[divIdx],
                        fSophieGermain ? 1 : 2);
                return false;
...
bool sieveResultsTest(
    ...
    for (unsigned wordIdx = 0; wordIdx < sieveWords; wordIdx++) {
        ...
        for (unsigned firstLayer = 0; firstLayer <= layersNum - chainLength;
             firstLayer++) {
            uint32_t mask = 0;
            ...
            if (mask != 0xFFFFFFFF) {
                for (unsigned bit = 0; bit < 32; bit++) {
                    if ((~mask & (1 << bit))) {
                        mpz_class candidateMultiplier =
                            (mpz_class)(sieveSize + wordIdx * 32 + bit)
                            << firstLayer;
                        mpz_class chainOrigin =
                            fixedMultiplier * candidateMultiplier;
        ...
        for (unsigned firstLayer = 0; firstLayer <= layersNum - bitwinLayers;
             firstLayer++) {
            uint32_t mask = 0;
...
void hipMultiplyBenchmark(
    hipFunction_t* kernels,
    unsigned groupsNum,
    ...
    void* squaringArguments[] = {
        &m1._deviceData, &mR._deviceData, &elementsNum};
    void* multiplicationArguments[] = {
        &m1._deviceData, &m2._deviceData, &mR._deviceData, &elementsNum};
    ...
    if (isSquaring) {
        for (unsigned i = 0; i < elementsNum; i++) {
            ...
            for (unsigned j = 0; j < MulOpsNum; j++) {
                ...
                memcpy(
                    Operand1,
                    target + mulOperandSize,
                    mulOperandSize * sizeof(uint32_t));
            }
        }
    } else {
        for (unsigned i = 0; i < elementsNum; i++) {
...
void hipMultiplyBenchmarkOCL(
    ...
    if (mulOperandSize == 256 / 32) {
        ...
        LOG_F(
            ERROR,
            "Can't multiply %u-size operands on HIP device",
            mulOperandSize * 32);
        return;
    }
    ...
    // OpenCL-compatible CPU reference: copy LOWER half (not upper half like
    // CUDA)
    for (unsigned i = 0; i < elementsNum; i++) {
        unsigned gmpLimbsNum = cpuM1[i].get_mpz_t()->_mp_size;
        mp_limb_t* Operand1 = cpuM1[i].get_mpz_t()->_mp_d;
        ...
        for (unsigned j = 0; j < 512; j++) {
            mpn_mul_n((mp_limb_t*)target, Operand1, Operand2, gmpLimbsNum);
            // Copy lower half (OpenCL approach) - this matches what the GPU
            // kernel does
    ...
    LOG_F(
        INFO,
        "Multiply %u bits: GPU %.0lf Mops/s, CPU %.0lf Mops/s (%.2fx faster)",
        mulOperandSize * 32,
        gpuMopsPerSec,
        cpuMopsPerSec,
        speedup);
...
void hipFermatTestBenchmark(
    ...
    for (unsigned i = 0; i < elementsNum; i++) {
        ...
        if (rand() % 16 == 0) {
            numbers[i * operandSize + operandSize - 2] =
                numbers[i * operandSize + operandSize - 1];
    ...
    void* arguments[] = {
        &numbers._deviceData, &gpuResults._deviceData, &elementsNum};
...
void hipHashmodBenchmark(
    ...
    for (unsigned i = 0; i < iterationsNum; i++) {
        ...
        for (unsigned i = 0; i < hashmod.count[0]; i++) {
            ...
            mpz_import(
                mpzRealPrimorial.get_mpz_t(), 2, -1, 4, 0, 0, &realPrimorial);
            primorialIdx = std::max(mPrimorial, primorialIdx) - mPrimorial;
            mpz_class mpzHashMultiplier =
                allPrimorials[primorialIdx] / mpzRealPrimorial;
    ...
    LOG_F(
        INFO,
        " MHash per second: %.3lf",
        iterationsNum * numhash / (double)totalTime);
    LOG_F(
        INFO,
        " Hash per iteration: %.3lf (%.6lf %%)",
        averageHashes,
        averageHashes * 100 / numhash);
    ...
    LOG_F(
        INFO,
        " Average hash multiplier size: %.3lf",
    ...
    for (unsigned i = 0; i < 20; i++) {
        if (phashCount[i]) {
            LOG_F(
                INFO,
                "   Hashed with primorial %u is %.3lf%%",
                i,
                phashCount[i] / (double)hashes * 100.0);
        }
    }
}

void hipSieveTestBenchmark(
    hipFunction_t* kernels,
    unsigned defaultGroupSize,
    unsigned groupsNum,
    ...
    while (foundHashNum < 64) {
        ...
        for (unsigned i = 0; i < hashmod.count[0]; ++i) {
            ...
            primorialIdx = std::max(mPrimorial, primorialIdx) - mPrimorial;
            mpz_class mpzHashMultiplier =
                allPrimorial[primorialIdx] / mpzRealPrimorial;
            unsigned hashMultiplierSize =
                mpz_sizeinbase(mpzHashMultiplier.get_mpz_t(), 2);
            mpz_import(
                mpzRealPrimorial.get_mpz_t(), 2, -1, 4, 0, 0, &realPrimorial);
    ...
    for (unsigned i = 0; i < count; i++) {
...

        {
            ...
            HIP_SAFE_CALL(hipModuleLaunchKernel(
                mSieveSetup,
                mConfig.PCOUNT / defaultGroupSize,
                1,
                1,
                defaultGroupSize,
                1,
...

        {
            ...
            HIP_SAFE_CALL(hipModuleLaunchKernel(
                mSieveSearch,
                (mConfig.SIZE * mConfig.STRIPES / 2) / 128,
                1,
                1,
                128,
                1,
        ...
        if (checkCandidates) {
            ...
            double coeff =
                fabs(n320 + n352 - multipliers.size()) / multipliers.size();

            if (coeff <= 0.01) {
                LOG_F(
                    INFO,
                    " * [%s] found candidates by CPU: %u by GPU: %u",
                    coeff <= 0.01 ? "OK" : "FAILED",
                    (unsigned)multipliers.size(),
                    n320 + n352);
                LOG_F(
                    INFO,
                    " * [%s] invalid candidates: %u",
                    !invalidCount ? "OK" : "FAILED",
                    invalidCount);
                LOG_F(
                    INFO,
                    " * [%s] CPU/GPU candidates difference: %u",
                    !diff ? "OK" : "FAILED",
                    diff);
            } else {
                LOG_F(
    ...
    if (!checkCandidates) {
        ...
        uint64_t bitsInSieve =
            mConfig.SIZE * 32 * mConfig.STRIPES / 2 * mConfig.WIDTH;
        ...
        LOG_F(
            INFO,
            " * candidates per second: %.3lf",
            (n320 + n352) / (totalTime / 1000000.0));
        LOG_F(
            INFO,
            " * candidates per iteration: %.2lf (%.2lf 320bit, %.2lf 352bit)",
            (double)(n320 + n352) / count,
            (double)n320 / count,
            (double)n352 / count);
        LOG_F(
            INFO,
            " * 320bit/352bit ratio: %.3lf/1",
...
void hipRunBenchmarks(
    ...
    for (unsigned i = 0; i < CUDAKernelsNum; i++)
        HIP_SAFE_CALL(
            hipModuleGetFunction(&kernels[i], module, gCUDAKernelNames[i]));
    ...
    // Get miner config
    {
        ...
        HIP_SAFE_CALL(hipModuleLaunchKernel(
            kernels[CUDAKernelGenConfig], 1, 1, 1, 1, 1, 1, 0, NULL, args, 0));
        HIP_SAFE_CALL(hipDeviceSynchronize());
    ...
    hipMultiplyBenchmark(
        kernels.get(), computeUnits * 4, 320 / 32, 262144, true);
    hipMultiplyBenchmark(
        kernels.get(), computeUnits * 4, 320 / 32, 262144, false);
    hipMultiplyBenchmark(
        kernels.get(), computeUnits * 4, 352 / 32, 262144, true);
    hipMultiplyBenchmark(
        kernels.get(), computeUnits * 4, 352 / 32, 262144, false);
    ...
    hipHashmodBenchmark(
        kernels.get(), defaultGroupSize, 0, allPrimorials, mPrimorial);
    hipSieveTestBenchmark(
        kernels.get(),
        defaultGroupSize,
        computeUnits * 4,
        allPrimorials,
