...
__device__ void mulProductScan320to96(
    uint32_t* out,
    const uint32_t* op1,
    const uint32_t* op2);
__device__ void mulProductScan320to128(
    uint32_t* out,
    const uint32_t* op1,
    const uint32_t* op2);
__device__ void mulProductScan320to192(
    uint32_t* out,
    const uint32_t* op1,
    const uint32_t* op2);
__device__ void mulProductScan352to96(
    uint32_t* out,
    const uint32_t* op1,
    const uint32_t* op2);
__device__ void mulProductScan352to128(
    uint32_t* out,
    const uint32_t* op1,
    const uint32_t* op2);
__device__ void mulProductScan352to192(
    uint32_t* out,
    const uint32_t* op1,
    const uint32_t* op2);
__device__ void mulProductScan352to32(
    uint32_t* out,
    const uint32_t* op1,
    uint32_t M);

__device__ unsigned divide512to352reg(
    uint32_t dv0,
    uint32_t dv1,
    uint32_t dv2,
    uint32_t dv3,
...
__device__ unsigned divide480to320reg(
    uint32_t dv0,
    uint32_t dv1,
    uint32_t dv2,
    uint32_t dv3,
...
extern "C" __global__ void getconfig(config_t* conf) {
    config_t c;
    c.N_ = N;
    c.SIZE_ = SIZE;
    c.STRIPES_ = STRIPES;
...
__device__ uint32_t
getFromBitfield(const uint32_t* ptr, unsigned bitOffset, unsigned bitSize) {
    union {
        uint2 v32;
        uint64_t v64;
    } data;
...
__device__ void FermatTest352(const uint32_t* e, uint32_t* redcl) {
    ...
    int remaining = divide512to352reg(
        0,
        0,
        0,
        0,
...
__device__ void FermatTest320(const uint32_t* e, uint32_t* redcl) {
    ...
    uint32_t inverted = invert_limb(e[0]);
    uint32_t q[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    int remaining = divide480to320reg(
        0,
        0,
        0,
        0,
...
extern "C" __global__ void setup_fermat(
    uint32_t* fprimes,
...
#pragma unroll
    ...
    uint32_t modifier =
        (info.type == 1 || (info.type == 2 && (info.chainpos & 1))) ? 1 : -1;

    // WORKAROUND: ROCm/HIP compiler optimization bug
    // Without volatile access, the compiler incorrectly optimizes away writes
    // inside mulProductScan352to32 This forces the compiler to treat m[] as
    // having side effects
...
extern "C" __global__ void check_fermat(
    fermat_t* info_out,
    uint32_t* count,
...
__device__ uint32_t
mod32(uint32_t* data, unsigned size, uint32_t* modulos, uint32_t divisor) {
    uint64_t acc = data[0];
...
extern "C" __global__ void setup_sieve(
    uint32_t* offset1,
    uint32_t* offset2,
...
#pragma unroll
    ...
    for (uint32_t layer = 0; layer < WIDTH; ++layer) {
        ...
        nFixedInverse = (nFixedInverse & 0x1) ? (nFixedInverse + nPrime) / 2
                                              : nFixedInverse / 2;
    }
}
